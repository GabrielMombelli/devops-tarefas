...
- A cada `push` na branch `master`, o GitHub Actions executa automaticamente:
  ...
  3. Sobe os serviços e testa se as rotas reais respondem (`/users`, `/tasks`)
  4. Simula o deploy
...
- O monitoramento de disponibilidade é feito pelo **UptimeRobot**, uma ferramenta real de mercado, em vez de uma rota `/health` própria
- O UptimeRobot faz requisições periódicas às URLs públicas dos serviços e do frontend
- Caso algum deles caia, o UptimeRobot detecta a indisponibilidade e pode notificar por email
- Isso demonstra monitoramento de uptime feito por uma ferramenta externa que observa o sistema de fora, como ocorre em ambientes de produção reais
...
│   ├── src/
│   │   ├── index.js              # Entrada da aplicação, rotas e CORS
